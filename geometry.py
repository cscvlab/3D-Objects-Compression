...
class PointSampler(): 
    ...
    def _normalDist(self, V):
        ...
        if self._std > 0.0:
            # V += np.random.rand(V.shape[0],V.shape[1]) * self._std
            # return V
...
class ImportanceSampler():
    ...
    def sample(self, N):
        ...
        s = self.sdf.query(U)
        I = self._subsample(s,int(N*0.009))

        R = np.random.choice(len(U), int(N*0.001))
        #/media/cscv/cscvlab128/ShapeNetCore.v2.zipS = U[I,:]
        S = np.concatenate((U[I,:],U[R, :]), axis=0)
        #S = U[I,:]
        return S
...
class Mesh():
    ...
    def _loadMesh(self, fp, doNormalize):
        ...
        # meshfix = pymeshfix.MeshFix(np.array(self._V),np.array(self._F))
        #
        # meshfix.repair()
        # meshfix.plot()
        # self._F = igl.eigen.MatrixXi(meshfix.f)
        # self._V = igl.eigen.MatrixXd(meshfix.v)
        if doNormalize:
            self._normalizeMesh()
